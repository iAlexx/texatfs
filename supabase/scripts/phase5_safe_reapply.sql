-- =============================================================================
-- Safe to run in Supabase SQL Editor when phase5 migration was already applied.
-- Use this instead of re-running 20260517120000_phase5_subscription_licensing.sql
-- (which fails with: type "license_duration_months" already exists)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS license_keys_is_used_idx ON public.license_keys(is_used);
CREATE INDEX IF NOT EXISTS license_keys_used_by_id_idx ON public.license_keys(used_by_id);
...
ALTER TABLE public.users
  ...
  ADD COLUMN IF NOT EXISTS license_key_id TEXT,
  ADD COLUMN IF NOT EXISTS registered_via TEXT NOT NULL DEFAULT 'seed';

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'users_license_key_id_fkey'
  ) THEN
    ALTER TABLE public.users
      ADD CONSTRAINT users_license_key_id_fkey
      FOREIGN KEY (license_key_id) REFERENCES public.license_keys(key) ON DELETE SET NULL;
  END IF;
END
$$;

ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_parent_role_chk;
ALTER TABLE public.users
...
CREATE INDEX IF NOT EXISTS users_subscription_end_date_idx
  ON public.users(subscription_end_date)
  WHERE subscription_end_date IS NOT NULL;

CREATE INDEX IF NOT EXISTS users_license_key_id_idx ON public.users(license_key_id);

ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_licensed_master_chk;
ALTER TABLE public.users
...
-- Functions (always safe to replace)
...
CREATE OR REPLACE FUNCTION public.redeem_license_key(p_key TEXT, p_user_id UUID)
RETURNS TIMESTAMPTZ
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  SET is_used = TRUE, used_by_id = p_user_id, used_at = now()
  WHERE key = p_key;

  UPDATE public.users
  SET subscription_end_date = v_end, license_key_id = p_key, updated_at = now()
  WHERE id = p_user_id;
...
-- RLS + policies (drop first to avoid duplicate policy errors)
...
CREATE POLICY license_keys_select_own ON public.license_keys
  ...
  USING (used_by_id = public.current_app_user_id());

DROP POLICY IF EXISTS api_snapshots_select_subscribed ON public.api_snapshots;
CREATE POLICY api_snapshots_select_subscribed ON public.api_snapshots
  AS RESTRICTIVE FOR SELECT TO authenticated
  USING (public.is_subscription_active(user_id));

DROP POLICY IF EXISTS daily_ledgers_select_subscribed ON public.daily_ledgers;
...
CREATE POLICY daily_ledgers_update_subscribed ON public.daily_ledgers
  ...
  USING (
    public.can_view_user(user_id) AND status = 'open'
...
DROP POLICY IF EXISTS transactions_select_subscribed ON public.transactions;
CREATE POLICY transactions_select_subscribed ON public.transactions
  AS RESTRICTIVE FOR SELECT TO authenticated
  USING (public.is_subscription_active(user_id));

DROP POLICY IF EXISTS transactions_insert_subscribed ON public.transactions;
