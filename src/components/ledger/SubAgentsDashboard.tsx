...
export function SubAgentsDashboard({
  ...
  return (
    <div className="space-y-5 pb-4">
      {/* Hero */}
      <motion.section
        className="relative overflow-hidden rounded-3xl border border-violet-500/25 bg-gradient-to-br from-violet-950/80 via-[#120818] to-obsidian p-5 shadow-[0_0_40px_rgba(139,92,246,0.12)]"
        initial={{ opacity: 0, y: 12 }}
        animate={{ opacity: 1, y: 0 }}
      >
        <div className="pointer-events-none absolute -left-8 -top-8 h-32 w-32 rounded-full bg-violet-600/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-6 -right-6 h-24 w-24 rounded-full bg-rose-600/15 blur-2xl" />
        ...
        <div className="relative flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="text-2xl font-bold text-foreground">{ar.agentsHeroTitle}</h2>
          ...
          {onRefresh ? (
            <Button
              ...
              variant="ghost"
              size="icon"
              className="shrink-0 text-steel-400 hover:text-violet-300"
              onClick={onRefresh}
              disabled={refreshing}
            >
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="h-12 rounded-2xl border-white/[0.08] bg-obsidian/70 pr-11 text-sm backdrop-blur-md placeholder:text-steel-600"
        />
      </div>
      ...
      <div className="space-y-3">
        {visible.length === 0 ? (
          <div className="rounded-2xl border border-white/[0.06] px-4 py-14 text-center text-sm text-steel-500">
...
function AgentCard({
  ...
  return (
    <motion.button
      type="button"
      onClick={onOpen}
      className="w-full rounded-2xl border border-white/[0.07] bg-gradient-to-l from-white/[0.03] to-transparent p-4 text-right transition-colors hover:border-violet-500/30 hover:bg-violet-500/[0.04]"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.03 * Math.min(index, 12) }}
    >
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600/30 to-rose-600/20 text-xs font-bold text-violet-200 ring-1 ring-violet-500/30">
          {roleCode}
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            {wa?.group_exists ? (
              ...
              <span className="rounded-full bg-steel-800/80 px-2 py-0.5 text-[9px] text-steel-500">
