...
export function LuxuryBalanceCard({
  ...
  return (
    ...
    >
      <motion.div layout className="relative z-10 flex items-start justify-between gap-3">
        <div>
        ...
        <motion.div
          className="rounded-full border border-gold/30 bg-gold/10 px-2 py-0.5 text-[9px] text-gold"
          layout
        >
          NSP
        </motion.div>
      ...
      >
        <motion.p className="text-center text-3xl embossed-gold md:text-4xl" layout>
          <AnimatedValue value={value} />
        </motion.p>
        <BalanceOrientationLabel value={value} showAmount={false} size="lg" />
      </motion.div>
    </motion.section>
  );
}
